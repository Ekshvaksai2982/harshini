...
app.post('/upload', upload.single('resume'), async (req, res) => {
  ...
  try {
    ...
    // Specify the path to the job roles dataset
    const jobRolesFilePath = path.join(__dirname, 'jobrolespskillsframeworks.xlsx');
    const excelData = readExcelData(jobRolesFilePath);
    ...
    // Specify the absolute path to the backend folder on the C: drive
    const backendDir = 'C:\\proj_JOBSINLINE\\backend';
    if (!fs.existsSync(backendDir)) {
      fs.mkdirSync(backendDir, { recursive: true }); // Create the directory if it doesn't exist
    }

    const userDataFilePath = path.join(backendDir, 'userdata.xlsx');
    appendToExcel(userDataFilePath, excelDataRow);

    fs.unlinkSync(filePath); // Delete the uploaded file after processing
    res.json(response);
  } catch (error) {
    console.error('Error processing request:', error.message);
    if (fs.existsSync(filePath)) fs.unlinkSync(filePath); // Clean up uploaded file on error
